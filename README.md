...
| `RESEND_WELCOME_FROM` | No | Verified `From` (e.g. `Alpha Investor <help@unicorn-code.com>`). Sends a welcome email after a **new** signup; omitted = no email |
| `RESEND_WELCOME_REPLY_TO` | No | If set, `Reply-To` for that email (e.g. your Gmail or `help@unicorn-code.com`) so answers land in your inbox instead of only the `From` address |
