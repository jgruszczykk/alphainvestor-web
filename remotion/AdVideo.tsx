...
function PhoneStage({ Screen, tab, slam = false }: { Screen: ComponentType; tab: string; slam?: boolean }) {
...
function Scene({ Screen, tab, line, pop, slam = false, dx = 190, dy = -470 }: { Screen: ComponentType; tab: string; line: string; pop: ReactNode; slam?: boolean; dx?: number; dy?: number }) {
  const { durationInFrames } = useVideoConfig();
  return (
    <AbsoluteFill>
      <PhoneStage Screen={Screen} tab={tab} slam={slam} />
...
export function AdVideo() {
  return (
    <AbsoluteFill style={{ background: "#000" }}>
      ...
      <Sequence from={86} durationInFrames={78}><Scene slam Screen={WalletScreen} tab="wallet" line="See everything you're building." pop={GreenPop("▲ +12.48%")} /></Sequence>
      <Sequence from={164} durationInFrames={78}><Scene Screen={InstrumentScreen} tab="watchlist" line="Every price. Every move." pop={GreenPop("NVDA ▲ +128%")} dx={-190} dy={-460} /></Sequence>
      <Sequence from={242} durationInFrames={78}><Scene Screen={AiScreen} tab="wallet" line="Know what actually matters." pop={AmberPop("✦ Mixed signals")} /></Sequence>
      <Sequence from={320} durationInFrames={78}><Scene Screen={LensesScreen} tab="watchlist" line="Every holding, judged." pop={GreenPop("Momentum 4/5")} dx={-190} dy={-460} /></Sequence>

      <Sequence from={398} durationInFrames={68}><CTA /></Sequence>
    </AbsoluteFill>
  );
}
